...
UNIlearn Maharashtra is the official mobile application for the Maharashtra State government's UNIlearn e-learning platform (Target Users
...
Design Principles
- No External Browser Redirects — All navigation stays within the app using <WebView />- API-First — All data (courses, grades, badges, certificates) is fetched live from the Moodle server using JSON REST APIs.
- Context over Redux — AuthContext- Decoupled Navigation — A shared navigationRef---
...
OfficialDrawer.js// src/navigation/navigationRef.js
...
- Base URL: https://mh.unilearn.org.in/webservice/rest/server.php- Format: json; Auth: Moodle token in wstoken- Handles: network errors, Moodle exception responses, token expiry
...
- Use lineHeight- Use minHeight---

9. Theming & Dark Mode

Managed by ThemeContext.jsSample theme tokens:
...
2. App calls core_user_get_users_by_field3. Token and user profile stored in AuthContext4. On next launch, stored token is checked; if valid, user is auto-logged in
5. Logout clears both AuthContextForgot Password opens a native in-app modal — no browser redirect.

Role detection: AuthContext---

11. SCORM Support

Full SCORM 1.2 compliance via ScormPlayerScreen.js1. SCORM package URL loaded into a WebView
2. JavaScript bridge injects a custom API3. SCORM content calls LMSInitialize, LMSSetValue, LMSGetValue, LMSFinish
4. Native app receives via onMessageOffline: Progress queued in AsyncStorage and retried on reconnect. Unsupported server functions are gracefully discarded.

12. Accessibility

Feature - Implementation

Font Scaling - Global fontScale multiplier (0.85–1.45x / 85%–145%) via ThemeContext
...
- Bundle ID: in.org.unilearn.mh- EAS Project ID: 3add74d6-d78f-4af6-8556-d6580af644d3- Minimum Permissions: INTERNET, ACCESS_NETWORK_STATE
...
UniLearnLogo.js
Renders the branded UNIlearn logo with a "DASHBOARD" pill button that opens the Maharashtra State Analytics screen via navigationRefOfficialTopHeader.js
...
OfficialDrawer.js
Side navigation drawer: user profile, full navigation links, logout, language switcher, theme toggle. Uses navigationRef---
...
SCORM Offline Queue
The server does not support mod_scorm_insert_tracksDevanagari Rendering
Avoid letterSpacing---
...
- Node.js >= 18
- npm >= 9
- Expo CLI (npm install -g expo-cli- Android Studio (for emulator) or physical device with Expo Go
